...
**TL;DR:** two Hugging Face references for Gemma 4 12B. `test_data/gemma4-12b-hf-golden.safetensors`
...
revision pins the payload. The window fixture records only the source repo and revision: its gate
validates the running checkpoint against the base fixture's hashes, then requires the two fixtures
to name the same revision, so one set of hashes covers both. **Transformers 5.11.0** is verified to load `gemma4_unified`; the
