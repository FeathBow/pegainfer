...
materialization — with observable semantics frozen rather than a closed pairing. Serving staged in two
steps as planned — one unreclaimed pool first, then the paged window with batching and long
context — and both have shipped. Checkpoint
...
it. The contract first named riding the paged-prefill kernel as the way out; **what shipped splits
the group instead**. `global_split_factor` accepts a group that is already dispatchable (1, 2, 3, 4
or 8) whole, and halves a group of twice one of those over a single KV head into two pseudo-requests
— an exact memory identity only because MQA gives every query head the same KV head. Anything else
fails loud. Decode then runs `paged_attention_batch_decode_split_kv_hd512_into` on the split, so the
full-attention group has a real decode kernel and is captured like any other. The ratio is still
**per rank, not per config**: query heads shard with the world size while a single KV head can only
be replicated, so the same group is 16 at TP1, 8 at TP2, 4 at TP4, and lands back inside the
compiled decode set above TP1 — where the split factor returns 1 and the whole path disappears.
Backend selection, GQA validation and graph capture must all key off the resolved per-rank
mapping.
...
## The budget as it shipped

The two pools are sized at start-up from the ceiling and the slot count, and the engine refuses to
start rather than admitting a request it cannot schedule. In pages, with `W` the window rounded up
plus one, `C` the ceiling, `S` the decode slots and `E` the prefix-cache entry count:

```
local  = (S - 1) x W + transient + 1 + E x W
global = S x C_pages + 1 + E x entry_global_pages

transient = C_pages                                   whole-prompt steps
          = W + chunk_pages + (MIX_MAX_PROMPTS - 1)   under the chunked walk
```

That second line is the whole reason a raised ceiling is possible at all: with whole-prompt steps
the local pool carries a full-context transient, which at the 31B checkpoint's own ceiling is tens
of GiB and fails at start-up. The chunked walk bounds every scan, and the engine enforces the
dependency — a ceiling above the default without `PEGAINFER_MIX_CHUNK_TOKENS` is refused, not
silently degraded.

Two consequences worth stating because they are counter-intuitive from the design text above.
The local pool does **not** grow with the ceiling, only with slots, window and chunk width; the
global pool grows with `S x C`, so concurrency and context trade against each other directly. And a
wider chunk costs local pages: at 31B, chunk 8192 puts the local pool at 581 pages where chunk 2048
puts it at 197, which is 7.1 GiB against 2.4 GiB.
...
3. **An ownership path for releasing a live request's oldest blocks.** ~~Nothing exposes one
   today.~~ **This landed.** `SlidingLocalKv::advance_and_release` drains the oldest per-page
   reservations of a live request, and `GemmaServe::advance_local` calls it after forward progress.
   The store-side elements this item bundled it with — an interval index, a dual-family checkpoint
   hold, a host tier keyed by content hash — did not land and were not needed for it: front
   reclamation of the sliding family is local to the request's own reservation list. Reading them
   as prerequisites was the error; they belong to prefix sharing and offload, which remain
   fail-closed under behaviour 4.
...
6. ~~FP8 for the global family~~ — **the opposite shipped, and the ladder was wrong about which
   family to reach for first.** `PEGAINFER_KV_FP8=local` stores the *sliding* family's K and V as
   e4m3 at scale 1.0, halving the local pool; the global family stays bf16. The ranking above
   argued from the linear term past 20k positions, which is sound arithmetic about capacity but
...
The knobs that shipped alongside are not on the list above because the list was written against
capacity, and these were written against admission and latency. All are opt-in with bytewise
identical serving when unset: `PEGAINFER_MIX_CHUNK_TOKENS` bounds a mixed admission's prompt rows
per step, which is what turns the local pool's transient term from a whole context into a window
plus a chunk and is the precondition for any ceiling above the default;
`PEGAINFER_MAX_CONTEXT` raises the serving ceiling to the checkpoint's own and refuses to start
without the chunked walk; `PEGAINFER_DECODE_SLOTS` trades concurrency for context by scaling the
global pool; `PEGAINFER_PREFIX_CACHE` is the amendment in behaviour 4 realised;
`PEGAINFER_ADMIT_COALESCE_MS` and `PEGAINFER_ASYNC_PREFILL` touch admission timing rather than the
cache. The contract below held through all of them without amendment beyond the one already
recorded.

## Excluded

- **Quantised KV at load**: the NVFP4 checkpoints declare an FP8 KV cache. The contract said an
  unsupported declared KV dtype would fail closed naming the scheme; **it does not, and nothing
  reads that field**. `probe.rs` validates `attention_k_eq_v`, `global_head_dim` and
  `num_kv_shared_layers` and never looks at a KV dtype, so a checkpoint declaring one starts
  normally. What the storage actually is comes from `PEGAINFER_KV_FP8`, chosen by the operator and
  independent of the declaration. An operator should not read this line as a guarantee that an
  incompatible cache policy was rejected — nothing rejected anything. Adding the check is a small,
  separate change; until it lands, the declaration is ignored.
