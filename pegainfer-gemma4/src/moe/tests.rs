...
/// Marlin assigns adjacent routes to M tiles, so preserving route order within
/// an expert is part of the BF16 numeric contract.
#[test]
#[ignore = "requires a GPU"]
fn a_large_dispatch_preserves_route_order_within_an_expert() {
    const ROWS: usize = 300;
    const TOP_K: usize = 4;
    const EXPERTS: usize = 128;
    const TARGET_EXPERT: i32 = 7;

    let ctx = DeviceContext::new_with_device(0).expect("device");
    let routes = ROWS * TOP_K;
    let max_blocks = routes.div_ceil(BLOCK) + EXPERTS;
    let max_padded = max_blocks * BLOCK;
    let mut topk_idx = ctx.stream.alloc_zeros::<i32>(routes).expect("topk");
    ctx.stream
        .memcpy_htod(&vec![TARGET_EXPERT; routes], &mut topk_idx)
        .expect("upload topk");

    let mut sorted_token_ids = ctx
        .stream
        .alloc_zeros::<i32>(max_padded)
        .expect("sorted routes");
    let mut expert_ids = ctx
        .stream
        .alloc_zeros::<i32>(max_blocks)
        .expect("expert ids");
    let mut padded_total = ctx.stream.alloc_zeros::<i32>(1).expect("padded total");
    let mut expert_offsets = ctx
        .stream
        .alloc_zeros::<u32>(EXPERTS + 1)
        .expect("expert offsets");
    let mut expert_cursor = ctx
        .stream
        .alloc_zeros::<u32>(EXPERTS)
        .expect("expert cursor");
    marlin_moe_align_block_size(
        ...
        BLOCK,
        &mut MoeAlignScratch {
            sorted_token_ids: &mut sorted_token_ids,
            expert_ids: &mut expert_ids,
            num_tokens_post_padded: &mut padded_total,
            expert_offsets: &mut expert_offsets,
            expert_cursor: &mut expert_cursor,
        },
    )
    .expect("align routes");
    ...
    let sorted = ctx
        .stream
        .clone_dtoh(&sorted_token_ids.slice(0..routes))
        .expect("download sorted routes");
    let mismatch = sorted
        .iter()
        .enumerate()
        .find(|(route, value)| i32::try_from(*route).expect("route offset") != **value);
    assert!(mismatch.is_none(), "first out-of-order route: {mismatch:?}");
}
