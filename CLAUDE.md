...
- `PEGAINFER_GLOBAL_ATTN` — gemma4 global-attention kernel and pool format: unset/`off` = the incumbent kernel on the split K|V pool (byte-identical serving); `tilelang` = the generated kernel on the same pool; `tilelang640` = the generated kernel on a folded pool whose rows hold K only where the rotation touches it and fold K's norm weight into the query, 640 columns per token per head instead of 1024 (both generated states need a build that carries the kernel; the pool's bytes are reported at start-up)
