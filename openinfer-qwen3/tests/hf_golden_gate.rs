...
//! different card. Instead this gate asserts openinfer lands within the *bf16
...
//!     few bf16 ULP), openinfer must pick the same token. Below that margin it is
//!     a genuine tie with no correct answer, so it is not enforced.
//!   * logprobs — on the head tokens, |openinfer − HF| is bounded in the mean
...
/// openinfer's chosen token may sit. ~3 bf16 ULP at typical logit magnitudes —
...
fn check_position(
    ...
    // is how far below HF's argmax (in HF's own logprobs) openinfer's pick sits.
    // A genuine bf16 tie differs by a ULP or two — small regret, fine. But a pick
    // HF scores clearly worse, or one absent from HF's top-K entirely (openinfer
    ...
    match hf_map.get(&pega_argmax) {
        None => stats.argmax_violations.push(format!(
            "seq {seq} pos {pos}: openinfer's argmax {pega_argmax} is absent from HF's top-{} — confidently wrong on a token HF does not rank",
            hf.len()
        )),
        Some(&hlp) if hf_top - hlp > MARGIN_TOL => stats.argmax_violations.push(format!(
            "seq {seq} pos {pos}: openinfer chose {pega_argmax}, which HF scores {:.4} nat below its own argmax (> {MARGIN_TOL} tie tolerance)",
            hf_top - hlp
        )),
        Some(_) => {}
    }
...
/// vector is openinfer's own top-1 logprob at each evaluated position — a
