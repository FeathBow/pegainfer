...
CUresult marlin_moe_align_block_size_cuda(
    const int* topk_idx, int* sorted_token_ids, int* expert_ids,
    int* num_tokens_post_padded, uint32_t* expert_offsets,
    uint32_t* expert_cursor, int active_tokens, int topk, int global_start,
    int local_experts, int block_size, int max_padded_tokens, int max_m_blocks,
    cudaStream_t stream);
...
CUresult kimi_moe_marlin_align_block_size_cuda(
  ...
  return marlin_moe_align_block_size_cuda(
      ...
      expert_offsets, expert_cursor, active_tokens, topk, global_start,
      local_experts, block_size, max_padded_tokens, max_m_blocks, stream);
}
